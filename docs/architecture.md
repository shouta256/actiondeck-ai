...
LangGraphとpgvectorはMVP時点では未実装で、Phase 2で導入予定です。MVPでは、まず処理の境界を明確にするため、`apps/api/app/agents` のPython workflowで `Triage -> Retrieval -> Planning -> Safety` を実装しています。
...
1. 評価ケース拡充 (10〜12件)
2. Python workflowをLangGraphへ移行する (conditional edge / fallback / approval gate を含む)
