...
MVPでは、まず処理の境界を明確にするため、`apps/api/app/agents` のPython workflowで `Triage -> Retrieval -> Planning -> Safety` を実装しています。Phase 2ではLangGraph移行を進めており、現在は `ignore` / `missing_info` routeを `triage -> safety` に短絡できるLangGraph runnerと、同じ評価ケースで比較するGraph評価モードを追加しています。pgvectorは今後の導入対象です。
...
1. 評価ケース拡充 (10〜12件)
2. Python workflowをLangGraphへ移行する (現在は `ignore` / `missing_info` のconditional edgeとGraph評価modeを追加済み。次に標準Agent RunをGraphへ切り替える)
