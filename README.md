...
MVPでは、まずAgent体験の縦スライスを優先するため、Pythonの小さなworkflowとseed evidence検索で実装しています。Phase 2ではLangGraph移行を進めており、現在は `ignore` / `missing_info` routeを短絡分岐できるLangGraph runnerと、移行確認用のGraph評価モードを追加しています。pgvectorは今後の導入対象です。
...
8. 評価ケースで既存workflow / LangGraph runner / Gemini生成の出力を測定する
