...
CREATE_EVIDENCE_ITEMS_TABLE_SQL = f"""
...
CREATE TABLE IF NOT EXISTS evidence_items (
  ...
  embedding vector({EVIDENCE_EMBEDDING_DIMENSIONS}) NOT NULL
);
...
async def _search_evidence_with_pgvector(
    ...
    try:
        await _ensure_evidence_items_table(connection)
        query_embedding = embed_evidence_text(query_text)
...
async def _ensure_evidence_items_table(connection: asyncpg.Connection) -> None:
    await connection.execute(CREATE_EVIDENCE_ITEMS_TABLE_SQL)
